...
class DateCalculator {
    public static void main(String args[]) {
        ...
        if(Year.isLeap(year)){
            System.out.println(getDate(year , 255));
        } else {
            System.out.println(getDate(year , 256));
        }
    }
    ...
    private static String getDate(int year , int day) {
        ...
        while(rem > dayofMonth[month]){
            rem -= dayofMonth[month++];
        }
        
        return String.format("%02d.%02d.%04d" , rem , month+1 , year);
    }
}